export function initTestimonianze() {
    ...
    if (!track || !slides.length)
        return;
    const perView = () => (window.innerWidth <= 860 ? 1 : 2);
    const total = () => Math.max(0, slides.length - perView() + 1);
    let current = 0;
    let timer = null;
    function go(idx) {
        const t = total();
        ...
        if (liveEl)
            liveEl.textContent = `Testimonianza ${current + 1} di ${t}`;
    }
    function start() { timer = setInterval(() => go(current + 1), 5500); }
