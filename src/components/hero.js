...
export function initHero() {
    ...
    const accentBubble = new THREE.Mesh(new THREE.SphereGeometry(0.18, 32, 32), new THREE.MeshPhysicalMaterial({
        color: 0xffd8b1,
        metalness: 0.05,
        roughness: 0.14,
        transmission: 0.7,
    ...
    scene.add(sculpture);
    const ring = new THREE.Mesh(new THREE.TorusGeometry(1.75, 0.03, 40, 120), new THREE.MeshStandardMaterial({
        color: 0xffc47e,
        metalness: 0.45,
        roughness: 0.2,
        transparent: true,
    ...
    scene.add(ring);
    const floor = new THREE.Mesh(new THREE.PlaneGeometry(20, 20), new THREE.ShadowMaterial({ opacity: 0.18 }));
